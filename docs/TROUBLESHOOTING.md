...
4. **Missing dependencies**
   ...
   pip install --force-reinstall "vps-fastsearch[rerank]"
   ```
...
2. **Check socket path**
   ```bash
   # Show config
   vps-fastsearch config show | grep socket_path
...
4. **Check database path**
   ...
   vps-fastsearch --db /path/to/correct.db search "query"
   ```
...
# Create diagnostic report
{
    ...
    echo "=== VPS-FastSearch ==="
    vps-fastsearch --version 2>/dev/null || pip show vps-fastsearch
    ...
    echo "=== Database ==="
    vps-fastsearch stats 2>/dev/null || echo "No database"

...
- **VPS-FastSearch >= 0.3.3** (includes critical memory and FTS5 fixes)
...
# Should show: vps-fastsearch 0.3.3 or higher
```
...
- Ensure VPS-FastSearch >= 0.3.3 with the `threads=2` fix
...
- Ensure VPS-FastSearch >= 0.3.3 with FTS5 query sanitization

...
- **VPS-FastSearch >= 0.3.2** (includes critical memory and FTS5 fixes)
...
# Should show: vps-fastsearch 0.3.2 or higher
```
...
- Ensure VPS-FastSearch >= 0.3.2 with the `threads=2` fix
...
- Ensure VPS-FastSearch >= 0.3.2 with FTS5 query sanitization

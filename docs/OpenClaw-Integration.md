...
- **VPS-FastSearch >= 0.3.4** (includes critical memory and FTS5 fixes)
...
# Should show: vps-fastsearch 0.3.4 or higher
```
...
- Ensure VPS-FastSearch >= 0.3.4 with the `threads=2` fix
...
- Ensure VPS-FastSearch >= 0.3.4 with FTS5 query sanitization
